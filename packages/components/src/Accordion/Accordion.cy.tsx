...
describe('Accordion', () => {
  ...
  it('renders all accordion items', () => {
    items.forEach((item, index) => {
      cy.get(`details:eq(${index}) summary`).should('contain.text', item.title);
      if (item.open) {
        cy.get(`details:eq(${index})`).should('have.attr', 'open');
        cy.get(`details:eq(${index}) p`).should('contain.text', item.body);
      } else {
        cy.get(`details:eq(${index})`).should('not.have.attr', 'open');
        cy.get(`details:eq(${index}) p`).should('not.be.visible');
      }
    });
  });
  ...
  it('toggles accordion items on summary click', () => {
    items.forEach((item, index) => {
      const summaryElement = cy.get(`details:eq(${index}) summary`);
      ...
      summaryElement.click();
      cy.get(`details:eq(${index})`).should('have.attr', 'open');
      cy.get(`details:eq(${index}) p`).should('be.visible');
    });
  });
